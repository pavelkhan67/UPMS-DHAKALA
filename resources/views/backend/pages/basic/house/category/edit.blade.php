            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-info">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form class="form-horizontal" id="houseCateogryForm" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                        ...
                                        <select required class="form-control select2" name="house_type_id" id="house_type_id">
                                            ...
                                            @if ($types)
                                                @foreach ($types as $type)
                                                    <option value="{{$type->id}}" @if($category->house_type_id == $type->id) selected @endif >{{$type->en_name}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" required name="en_name" value="{{$category->en_name}}" placeholder="House Category" class="form-control" id="en_name">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" required name="bn_name" value="{{$category->bn_name}}" placeholder="House Category Bangla" class="form-control" id="bn_name">
...
@push('script')
    <script>
        $(document).ready(function() {
            $(".select2").select2();
            $("#houseCateogryForm").on('submit', function(e) {
                e.preventDefault();
                let thisForm = $(this);
                $.ajax({
                    type: "POST",
                    url: "{{route('basic-settings.house-category.update', $category->id)}}",
