                        ...
                        <div class="card-body">
                            <table>
                                ...
                                <thead>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <h2 class="text-light bg-success bold">Birth Certificate</h2>
                                        </td>
                                ...
                                <tbody>
                                    <tr>
                                        <td colspan="3">
                                            <p class="text-justify line-height">This is to certify that <strong class="text-danger">{{ $certificate->user->name ?? '' }}</strong>
                                                Id no.: <strong class="text-danger">
                                                    {{ $certificate->user->system_id ?? '' }}</strong>
                                                son of <strong class="text-danger">{{ family_live_status($certificate->user->familyInfo->father_live_status ?? 0) }}
                                                    {{ $certificate->user->familyInfo->father_name ?? '' }}</strong> &
                                                <strong class="text-danger">{{ family_live_status($certificate->user->familyInfo->mother_live_status ?? 0) }}
                                                    {{ $certificate->user->familyInfo->mother_name ?? '' }}</strong>,
                                                Village: <strong class="text-danger">{{ $certificate->user->addressInfo->permanentVillage->en_name ?? '' }}</strong>,
                                                <strong class="text-danger">Road-{{ $certificate->user->addressInfo->permanent_road ?? '' }}</strong>,
                                                ...
                                                PO.:
                                                <strong class="text-danger">{{ $certificate->user->institute->union->name ?? '' }}</strong>,
                                                PS.
                                                : <strong class="text-danger">{{ $certificate->user->institute->union->thana->name ?? '' }}</strong>,
                                                Dist.: <strong class="text-danger">{{ $certificate->user->institute->union->thana->district->name ?? '' }}</strong>,
                                                is known to me for about long time.
                                                
                                            </p>
                                            <p>To the best of my knowledge, his/her Birth date is --. And he is a Bangladeshi.</p>
