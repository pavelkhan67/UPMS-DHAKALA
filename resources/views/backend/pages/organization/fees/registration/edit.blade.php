...
@push('style')
@endpush
@section('title', 'Organization Registration Fees Edit')
@section('content')
                            ...
                            <div class="tabl-responsive">
                                                ...
                                                <select class="form-control" name="tax_year_id">
                                                    ...
                                                    @if ($tax_years)
                                                        @foreach ($tax_years as $tax_year)
                                                            <option value="{{ $tax_year->id }}" {{$fee->tax_year_id ==  $tax_year->id ? 'selected' : '' }}>{{ $tax_year->name }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                                ...
                                                <select class="form-control" id="institute_type_id" name="institute_type_id">
                                                    ...
                                                    @if (count($institute_types))
                                                        ...
                                                        @foreach ($institute_types as $institute_type)
                                                            <option value="{{ $institute_type->id }}" {{$fee->institute_type_id ==  $institute_type->id ? 'selected' : '' }} >
                                                ...
                                                <select class="form-control" id="organization_category_id"
                                                    ...
                                                    @if (count($organization_categories))
                                                        @foreach ($organization_categories as $org_category)
                                                            <option value="{{ $org_category->id }}"  {{$fee->organization_category_id ==  $org_category->id ? 'selected' : '' }} >
                                                ...
                                                <select class="form-control" id="organization_subcategory_id"
                                                    ...
                                                    @if (count($organization_sub_categories))
                                                        @foreach ($organization_sub_categories as $organization_sub_category)
                                                            <option value="{{ $organization_sub_category->id }}" {{$fee->organization_subcategory_id ==  $organization_sub_category->id ? 'selected' : '' }} >
                                ...
                                <table class="table table-bordered text-center">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>1. </td>
                                            <td><input class="form-control text-left" value="{{$fee->name}}"
                                                    name="name"></td>
                                            <td><input class="form-control  text-right" value="{{$fee->category_a_fees ?? 0}}" min="0"
                                                    type="number" name="category_a_fees"></td>
                                            <td><input class="form-control  text-right" value="{{$fee->category_b_fees ?? 0}}" min="0"
                                                    type="number" name="category_b_fees"></td>
                                            <td><input class="form-control  text-right" value="{{$fee->category_c_fees ?? 0}}" min="0"
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#organizationFeesForm").on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('organizationA.registration-fees.update', $fee->id)}}",
